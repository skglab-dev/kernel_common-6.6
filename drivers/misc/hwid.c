...
static const char* product_names1[] =
{
	"shennong",
	"houji",
	"manet",
...
static const char* product_names2[] =
{
	"nuwa",
	"fuxi",
	"wangshu",
...
static const char* product_names3[] =
{
	"zeus",
	"cupid",
	"katyusha",
...
{
	uint project_id = project - 1;
	uint32_t soc_info_id = socinfo_get_id();

	if (soc_info_id <= 0x22C) {
		if (soc_info_id == 0x1C9) {
			if ( project_id < 12 )
				return product_names3[project_id];
		}
		else if (soc_info_id == 0x207) {
			if ( project_id < 6 )
				return product_names2[project_id];
		}
	}
	else {
		switch ( soc_info_id ) {
			case 0x22D:
			if ( project_id < 7 )
				return product_names1[project_id];
			break;
			case 0x266:
			if ( project == 8 )
				return "peridot";
			...
			case 0x26A:
			if ( project_id < 11 )
				return product_names[project_id];
			break;
		}
	}
	return "unknown";
