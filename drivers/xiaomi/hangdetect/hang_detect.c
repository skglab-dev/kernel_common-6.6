 ...
 * Copyright (C) 2024.
 */
#define pr_fmt(fmt) "hang_detect: "fmt
...
#ifdef CONFIG_MI_HANG_DETECT_LOG
...
static struct miscdevice Monitor_Hang_dev = {
	.minor = MISC_DYNAMIC_MINOR,
	.name = "Monitor_Hang",
	.fops = &Monitor_Hang_fops,
};
...
static int hang_detect_dump_thread(void *arg)
{
	...
	pr_notice("hang_detect dump thread exit.\n");
	return 0;
}
...
static void hang_task_utc(struct tm *tm)
{
...
static int hang_detect_thread(void *arg)
{
	...
	reset_hang_info();
	msleep(120 * 1000);
	pr_debug("hang_detect thread starts.\n");
	...
	while (1) {
		hang_task_utc(&tm);
		pr_err("hang_detect thread counts down %d:%d, status %d.[%ld-%02d-%02d %02d:%02d:%02d]\n",
...
void monitor_hang_kick(int lParam)
{
	if (lParam == 0) {
		...
		pr_info("hang_detect enabled %d\n", hd_timeout);
	}
	reset_hang_info();
}

#ifdef CONFIG_MI_HANG_DETECT_LOG
static void add_log_memory_to_dump_region(void *pa, void *va, char *name, size_t region_size)
{
	...
	md_entry.virt_addr = (uint64_t)va;
	md_entry.phys_addr = (uint64_t)pa;
...
static int __init monitor_hang_init(void)
{
	...
	add_log_memory_to_dump_region((void *)virt_to_phys(Hang_Info), (void *)Hang_Info, "hang_task", MAX_HANG_INFO_SIZE);
#endif
	...
	err = misc_register(&Monitor_Hang_dev);
	if (unlikely(err)) {
		pr_notice("failed to register Monitor_Hang_dev device!\n");
		return err;
	}
...
{
	misc_deregister(&Monitor_Hang_dev);
	kfree(thread_array);
#ifdef CONFIG_MI_HANG_DETECT_LOG
	/* kfree(NULL) is safe */
	kfree(Hang_Info);
