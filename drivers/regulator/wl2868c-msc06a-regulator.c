...
#define WL2868C_CLASS_NAME       "satellite_ldo"
...
static struct regulator_data reg_data[] = {
    { "wl2868c-msc06a-l1", "none", 1200000, 80000, 800000},
    { "wl2868c-msc06a-l2", "none", 1200000, 80000, 800000},
    { "wl2868c-msc06a-l3", "none", 3400000, 80000, 800000},
    { "wl2868c-msc06a-l4", "none", 1800000, 80000, 800000},
    { "wl2868c-msc06a-l5", "none", 1800000, 80000, 800000},
    { "wl2868c-msc06a-l6", "none", 1800000, 80000, 800000},
    { "wl2868c-msc06a-l7", "none", 2800000, 80000, 800000},
};
...
static int wl2868c_write(struct regmap *regmap, u16 reg, u8*val, int count)
{
    ...
    wl2868c_err("Writing 0x%02x to 0x%02x\n", *val, reg);
...
{
    int rc;
    wl2868c_err("Writing 0x%02x to 0x%04x with mask 0x%02x\n", val, reg, mask);
...
static int wl2868c_regulator_enable(struct regulator_dev *rdev)
{
    ...
    wl2868c_err("[%s][%d] regulator enable\n", fan_reg->rdesc.name, fan_reg->index);
    return 0;
...
static int wl2868c_regulator_disable(struct regulator_dev *rdev)
{
    ...
    wl2868c_err("[%s][%d] regulator disabled\n", fan_reg->rdesc.name, fan_reg->index);
    return 0;
}
    ...
    const char *name)
{
    ...
    wl2868c_err("try to register ldo %s\n", name);
    ...
    wl2868c_err("%s regulator register done\n", name);
    return 0;
}
    ...
    struct device_attribute *attr, char *buf)
{
    ...
    if (rc < 0) {
        ...
        wl2868c_err("WL2868C get Product ID: [%02x]\n", val);
...
{
    unsigned long ret;
    char device_drv_name[WL2868C_NAME_STR_LEN_MAX] = "wl2868c_ldo";
    ...
    if (alloc_chrdev_region(&(wl2868c_dev.dev_no), 0, 1,
            device_drv_name)) {
        wl2868c_err("[WL2868C] Allocate device no failed\n");
        return -EAGAIN;
    }
    ...
    if (wl2868c_dev.pcdev == NULL) {
        ...
        wl2868c_err("[WL2868C] Allocate mem for kobject failed\n");
        return -ENOMEM;
    }
    ...
    if (cdev_add(wl2868c_dev.pcdev, wl2868c_dev.dev_no, 1)) {
        wl2868c_err("Attatch file operation failed\n");
        unregister_chrdev_region(wl2868c_dev.dev_no, 1);
        return -EAGAIN;
    }
    ...
    if (IS_ERR(wl2868c_dev.pdevice)) {
        int ret = PTR_ERR(wl2868c_dev.pdevice);
        wl2868c_debug("[WL2868C] device_create failed, err = %d\n", ret);
        return ret;
    }
...
static const struct of_device_id wl2868c_dt_ids[] = {
    {
        .compatible = "willsemi,wl2868c-msc06a",
    },
    { }
};
