...
void dma_buf_ref_destroy(struct msm_dma_buf *msm_dma_buf)
{
	...
	mutex_lock(&msm_dma_buf->lock);
	list_for_each_entry_safe(r, n, &msm_dma_buf->refs, list) {
		list_del(&r->list);
		kfree(r);
	}
	mutex_unlock(&msm_dma_buf->lock);
}
...
int dma_buf_refs_show(struct seq_file *s, void *v)
{
	...
	ret = mutex_lock_interruptible(&msm_db_list.lock);
	if (ret)
		return ret;
...
static int proc_create_dma_buf_ref(void)
{
